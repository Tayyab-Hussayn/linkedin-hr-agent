...
## Recently Completed (March 28 2026)

- prompt_builder.py created at playwright/prompt_builder.py
  with 8 niches: hr_professional, digital_marketer,
  web_developer, ceo_founder, consultant, sales_professional,
  finance_professional, product_manager

- Flask endpoints added to action_server.py:
  GET  /api/client-profile/<client_id>
  PUT  /api/client-profile/<client_id>
  GET  /api/niches

- n8n workflow 02 updated — Build Generation Prompt node
  now fetches dynamic system prompt from Flask
  instead of hardcoded HR prompt logic

- posts table fixed: id column now has DEFAULT gen_random_uuid()

- End to end tested and confirmed working:
  workflow executes → Flask prompt fetched → AI generates
  → posts saved to DB → appear in dashboard queue

- Dynamic prompt system fully operational for all 8 niches
...
## DECIDED ARCHITECTURE (March 2026)

### Deployment Model: Client-Server with Desktop App

This is NOT a traditional web SaaS. The system is split
into two parts:

**Cloud Server (our VPS)**
- PostgreSQL — all client data, posts, plans
- Flask API — auth, data endpoints, prompt builder
- n8n — AI content generation, cron scheduling
- Ollama/Gemini — LLM for content
- Static file server — serves latest linkedin_actions.py for auto-update

**Client Machine (Tauri desktop app)**
- Dashboard UI (Next.js compiled into Tauri webview)
- queue_worker.py (Python sidecar — runs locally)
- linkedin_actions.py (auto-updated from server)
- Playwright + browser profile (local LinkedIn session)
- Auto-updater (checks server version on every launch)

### Why This Architecture
- Playwright runs on client's own machine = their own IP = no LinkedIn bans
- Each client's browser session is isolated on their machine
- LLM and DB stay on server = clients always depend on us = recurring revenue
- Scales to 1000 clients with zero extra server compute for publishing
- LinkedIn CSS changes: we fix linkedin_actions.py once on server,
  all clients auto-update silently within 24h

### Auto-Update System for linkedin_actions.py
Server exposes:
  GET /updater/version.json       → {"version": "1.0.4", "checksum": "abc123"}
  GET /updater/linkedin_actions.py → latest script

Client checks on every launch:
  1. Fetch version.json from server
  2. Compare with local version
  3. If different → download new script → verify checksum → replace → restart worker

### Auth System (lives on cloud server)
- Client opens Tauri app → login screen
- POST /auth/login → server validates → returns JWT token
- Token stored in Tauri secure store
- Every API call sends: Authorization: Bearer <token>
- Server extracts client_id from token → all data scoped automatically
- No client ever sees another client's data

### Build Order
Phase 1 — Cloud ready:
  ✅ 1. Auth system on Flask (login, register, JWT) — COMPLETE
  ✅ 2. Remove hardcoded hr-pro-001 — all endpoints use JWT client_id — COMPLETE
  🔲 3. Domain + Cloudflare tunnel → Flask live on internet — PENDING (need domain)
  🔲 4. systemd services — auto-start on boot — PENDING

Phase 2 — Desktop app:
  5. Tauri shell with Next.js dashboard inside
  6. Python sidecar (queue_worker + linkedin_actions) bundled
  7. Auto-updater for linkedin_actions.py
  8. Onboarding wizard (niche picker, profile setup)

Phase 3 — Polish:
  9. Code signing for distribution
  10. Full Tauri app auto-update
  11. Admin panel for managing all clients

### Current Status
- Phase 1: 50% complete (auth done, deployment pending)
- All services still running locally
- JWT auth system operational with multi-client isolation
- Backward compatibility maintained (hr-pro-001 works without auth)
...
1. ~~**Import workflow 02 v2** into n8n and test generation~~ ✅ DONE
